...
User.all.each do |user|
  ...
  while user.contacts.count < 50
    ...
    contact = User.find(User.where.not(id: -1).where.not(id: user.id).pluck(:id).sample) while contact.cards.count.zero?

    contact_card = Card.find(contact.cards.pluck(:id).sample)

    unless Connection.find_by(user_id: user.id, contact_id: contact.id, card_id: contact_card.id)
      Connection.create!(
        user_id: user.id,
    ...
    next if Connection.find_by(user_id: contact.id, contact_id: user.id, card_id: user_card.id)
